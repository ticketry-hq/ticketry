...
impl DjangoGeneration {
    ...
    fn leaf(self) -> &'static str {
        match self {
            ...
            Self::Final => schema::FINAL_DJANGO_LEAF,
...
pub async fn adopt(data_directory: &Path) -> Result<AdoptionEvidence, RunsPersistenceError> {
    ...
    if source == SourceClassification::RustOwned {
        // An already-owned store still gains the additive attempt columns this
        // build reads. Nothing else about its history is touched, so its
...
async fn classify(
    ...
    let generation = match migrations.last().map(String::as_str) {
        ...
        Some(schema::FINAL_DJANGO_LEAF) => DjangoGeneration::Final,
        _ => {
            return Err(incompatible(
...
async fn validate_manifest(
    ...
    // Columns this build reconciles additively are optional here. A store that
    // has not been reconciled yet is still a store this build can adopt, and a
    // store that already gained them is unchanged history.
    for (column, _) in schema::ATTEMPT_RECONCILED_COLUMNS {
        if attempt.contains(*column) {
            expected_attempt.insert((*column).to_owned());
        }
    }
...
async fn stable_digest(
    ...
    digest_agent_runs(database, &mut hasher).await?;
...
async fn digest_agent_runs(
    database: &impl ConnectionTrait,
    hasher: &mut Sha256,
) -> Result<(), RunsPersistenceError> {
    let installed = schema::columns(database, "agent_runs").await?;
    let expressions = AGENT_RUN_COLUMNS
        .iter()
        .map(|column| {
        ...
        .collect::<Vec<_>>();
    digest_expressions(database, "agent_runs", &expressions, hasher).await
}
    ...
    match generation {
        ...
        DjangoGeneration::Final => [
            ...
            &[
                ...
                schema::LEGACY_TERMINAL_DJANGO_LEAF,
                schema::MERGED_DJANGO_LEAF,
                "0016_agentrun_launch_reasoning",
                schema::FINAL_DJANGO_LEAF,
            ],
        ]
        .concat()
        .into_iter()
...
async fn digest_generation(
    ...
    if schema::columns(database, "agent_runs")
        .await?
        .contains("launch_unattended")
    {
        return Ok(DjangoGeneration::Final);
    }
    let row = database
        .query_one_raw(Statement::from_string(
            DbBackend::Sqlite,
    ...
    match leaf.as_str() {
        ...
        schema::FINAL_DJANGO_LEAF => Ok(DjangoGeneration::Final),
        _ => Err(incompatible(
...
async fn digest_table(
    ...
    let expressions = columns
        ...
        .collect::<Vec<_>>();
    digest_expressions(database, table, &expressions, hasher).await
}

async fn digest_expressions(
    database: &impl ConnectionTrait,
    table: &str,
    expressions: &[String],
    hasher: &mut Sha256,
