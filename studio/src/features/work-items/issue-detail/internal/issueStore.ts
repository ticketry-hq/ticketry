...
export const useIssueStore = create<IssueState>((set, get) => ({
  ...
  hydrateWorkItems(items) {
    ...
    set({ workItemsById, workItemIdByKey, childWorkItemIds, open, children });
  },
  ...
  async openIssue(keyOrId) {
    ...
    // after the user has had a chance to see it. The timer and controller are
    // shared because a drawer and the workspace can host the same issue.
    if (cached) {
      ...
      // deep-link/drawer convenience work non-blocking and off the paint path.
      ...
      refreshTimer = setTimeout(() => {
        ...
        void api.getWorkItem(cachedId, controller.signal)
          .then((detail) => {
