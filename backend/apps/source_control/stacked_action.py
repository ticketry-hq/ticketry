...
from apps.source_control.action_steps import STATUS_FAILED, ActionStep
...
def commit_and_push(checkout: Checkout) -> StackedOutcome:
    ...
    return _combined(committed, pushed)
...
def commit_and_push_module(module_id: str) -> StackedOutcome:
    ...
    return commit_and_push(module_checkout_for_action(module_id))


def _combined(
    committed: CommitOutcome, pushed: push.PushOutcome
) -> StackedOutcome:
...
def _status(committed: CommitOutcome, pushed: push.PushOutcome) -> str:
    ...
    if pushed.published:
        return (
            COMMITTED_AND_PUSHED if committed.status == COMMITTED else PUSHED
        )
