...
pub(super) fn assert_complete() {
    debug_assert_eq!(DOMAIN_OPERATIONS.len(), 3);
    debug_assert_eq!(
        DOMAIN_OPERATIONS[0].field,
        "worktree_pull_request_merge_prepare"
    );
    debug_assert_eq!(DOMAIN_OPERATIONS[1].field, "worktree_file_diff");
    debug_assert_eq!(DOMAIN_OPERATIONS[2].field, "module_file_diff");
...
mod tests {
    ...
    #[test]
    fn every_live_git_read_operation_is_registered() {
        assert_eq!(DOMAIN_OPERATIONS.len(), 3);
        assert_eq!(
            DOMAIN_OPERATIONS[0].field,
            "worktree_pull_request_merge_prepare"
        );
        assert_eq!(DOMAIN_OPERATIONS[1].field, "worktree_file_diff");
        assert_eq!(DOMAIN_OPERATIONS[2].field, "module_file_diff");
