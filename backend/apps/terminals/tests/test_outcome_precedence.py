...
async def test_a_late_provider_hook_cannot_resurrect_a_terminated_run(runtime):
    ...
    await sync_to_async(rust_port.apply_lifecycle_fact, thread_sensitive=True)(
        "run-late-hook", "working", later
    )
...
async def test_a_reload_snapshot_reconstructs_the_same_state_on_both_sides(runtime):
    ...
    lifecycle_at = (datetime.now(timezone.utc) + timedelta(seconds=1)).isoformat()
    await sync_to_async(rust_port.apply_lifecycle_fact, thread_sensitive=True)(
        "run-reloaded", "turn_start", lifecycle_at
    )
...
async def test_a_run_waiting_on_the_user_keeps_its_attention_state(runtime):
    ...
    lifecycle_at = (datetime.now(timezone.utc) + timedelta(seconds=1)).isoformat()
    await sync_to_async(rust_port.apply_lifecycle_fact, thread_sensitive=True)(
        "run-waiting", "awaiting_input", lifecycle_at
    )
    ...
    permission_at = (datetime.now(timezone.utc) + timedelta(seconds=2)).isoformat()
    await sync_to_async(rust_port.apply_lifecycle_fact, thread_sensitive=True)(
        "run-waiting", "permission_required", permission_at
    )
