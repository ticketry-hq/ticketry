...
import { docUrl } from "../agents/api/agentApi";
import type { DocTabState } from "../agents/types";
import { ApiError, saveDocument } from "../studio/lib/api";
...
function MarkdownDocViewer({
  ...
  useEffect(() => {
    ...
    void fetchMarkdown(doc, controller.signal)
      .then((loaded) => {
  ...
  async function reloadExternal(): Promise<void> {
    ...
    try {
      applyExternal(await fetchMarkdown(doc));
    } catch {
      setSaveError(true);
    } finally {
  ...
  async function compareExternal(): Promise<void> {
    setLoadingExternal(true);
    try {
      const loaded = await fetchMarkdown(doc);
