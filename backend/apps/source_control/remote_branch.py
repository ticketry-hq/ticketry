...
from apps.source_control.git_cli import run_git, run_git_capturing
...
def resolve_remote(repo_path: str, branch: str) -> Optional[str]:
...
def _commit_count(
    ...
    *,
    ...
    """How many commits this push would publish.
    ...
    if remote_sha is not None and fast_forward:
        return _rev_list_count(repo_path, f"{remote_sha}..HEAD")
    base = _base_ref(repo_path, remote=remote, base_branch=base_branch)
    if base is None:
        return _rev_list_count(repo_path, "HEAD")
    return _rev_list_count(repo_path, f"{base}..HEAD")


def _base_ref(
    repo_path: str, *, remote: str, base_branch: str
) -> Optional[str]:
...
def _rev_list_count(repo_path: str, spec: str) -> int:
    counted = run_git(
        ["rev-list", "--count", spec],
        cwd=repo_path,
