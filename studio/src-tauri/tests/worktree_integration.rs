...
use muxed_studio_lib::workspace::operations::{self, WorkspaceOperationJournal};
use muxed_studio_lib::worktree::integrate::{
    DeliveryOutcome, IntegrationDelivery, WorktreeIntegrateService, MAX_DELIVERY_BATCH,
};
use muxed_studio_lib::worktree::status::RepositoryLocks;
...
async fn install(database: &DatabaseConnection) {
    operations::schema::install(database)
        .await
