...
#[derive(Debug)]
pub(crate) struct CrashReportsRuntime {
...
pub(crate) enum CrashCollectionOutcome {
    None,
    ReportCollected,
}

impl CrashReportsRuntime {
    pub(crate) fn new(data_directory: &Path, latest_report: Option<PathBuf>) -> Self {
        Self {
...
#[tauri::command]
pub(crate) fn desktop_latest_crash_collection_outcome(
...
#[tauri::command]
pub(crate) fn desktop_reveal_crash_report_folder(
    window: tauri::WebviewWindow,
    state: tauri::State<'_, CrashReportsRuntime>,
) -> Result<(), String> {
    require_main_window(&window)?;
