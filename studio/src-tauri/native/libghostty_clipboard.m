...
static bool muxed_ghostty_read_clipboard(
    const muxed_ghostty_surface_owner_s *owner,
    ghostty_clipboard_e clipboard, void *state, NSPasteboard *pasteboard,
    muxed_ghostty_clipboard_completion_fn complete) {
  if (muxed_ghostty_owned_surface(owner) == NULL || state == NULL ||
      clipboard != GHOSTTY_CLIPBOARD_STANDARD || pasteboard == nil)
    return false;

  NSString *value = [pasteboard stringForType:NSPasteboardTypeString];
  return muxed_ghostty_complete_clipboard_text(owner, state, value, complete);
}
...
static bool runtime_read_clipboard(void *userdata, ghostty_clipboard_e clipboard,
  ...
  return muxed_ghostty_read_clipboard(
      owner, clipboard, state, [NSPasteboard generalPasteboard],
