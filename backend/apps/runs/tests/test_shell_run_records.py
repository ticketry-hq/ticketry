...
async def _insert_shell_run(run_id: str = "run-shell") -> AgentRun:
    ...
    await sync_to_async(rust_port.prepare_launch, thread_sensitive=True)(
        {
            "effectId": f"effect-{run_id}",
            "agentRunId": run_id,
            "requestId": run_id,
            "projectId": fixture_uuid("proj-1"),
            "issueId": _module_issue_id(),
            "scope": SHELL_SCOPE,
            "provider": None,
            "targetKind": "work_item",
            "targetId": _module_issue_id(),
        },
        {},
    )
    ...
    await sync_to_async(rust_port.prepare_launch, thread_sensitive=True)(
        {
            ...
            "requestId": run_id,
            "projectId": fixture_uuid("proj-1"),
            "issueId": _task_issue_id(),
            "scope": "task",
            "provider": "claude",
            "targetKind": "work_item",
            "targetId": _task_issue_id(),
        },
        {},
    )
