...
| 180 | The launcher menu anchors outside the scrolling tab strip, so opening it does not clip or scroll the strip. |
| 181 | The pinned Changes tab reads the worktree only once opened, lists modified, new, and deleted files with their own counts, and opens one file's working-tree diff in place. |
| 182 | Refresh re-reads the checkout, and invalidating the worktree-changes contract after a mutation repaints the panel without a manual refresh. |
| 183 | A task with no worktree and a failed checkout read each explain themselves inside the Changes tab instead of blanking it. |
| 184 | The module's own workspace pins the same Changes tab, reads the module base checkout only once opened, lists its changed files with their counts, presents the branch alone because a base checkout has no merge target, and opens one file's working-tree diff in the same viewer. |
| 185 | Refresh re-reads the module checkout, invalidating a task worktree in the same module leaves the module review untouched, and invalidating the module checkout repaints it. |
| 186 | A module with no readable checkout and a failed module read each explain themselves inside the Changes tab instead of blanking it. |
| 187 | The Changes tab's action footer states that hooks always run, lists the primary action's steps in run order before anything happens, and the commit-only action in its menu commits the whole change set without sending a path list, reports each step's own outcome with the commit's sha and generated subject, and re-reads the checkout afterwards. |
| 188 | A commit the repository's hooks refused shows the curated failure with the hook's captured output, commits nothing, and leaves the reviewed file list on screen. |
| 189 | A worktree that matches its last commit keeps the action footer's safety copy visible but offers no commit to run. |
| 190 | Commit & push reads nothing from the remote until it is asked, then confirms the branch, remote and commit count with no generated commit text, sends only the checkout once confirmed, reports all four steps' own outcomes in run order, and re-reads the checkout afterwards. |
| 191 | A diverged branch reports a failed push step telling the user to resolve it in a terminal, states that the commit it already made is safe in the worktree, and shows no git output. |
| 192 | A detached HEAD blocks the confirmation with its own precondition sentence and offers no button that could start the action. |
| 193 | A clean worktree still offers Commit & push, and a remote that already has the branch's last commit reports every step — the push included — as an explicit skip. |
| 194 | Reviewing a file's diff, confirming, and running the primary action commits, pushes, and opens a GitHub pull request in one ordered pass, reports all five steps' own outcomes in run order, sends only the checkout, surfaces the pull request URL, opens it in the system browser once, and re-reads the checkout afterwards. |
| 195 | A checkout on the repository's default branch is refused with its own precondition sentence, runs no step, opens no browser tab, and leaves the reviewed file list on screen. |
| 196 | A pull request GitHub refused reports a failed pull-request step with no provider output, states that the commit and the published branch both stand, and offers the pull request on its own — which reports every earlier step as an explicit skip. |
| 197 | A failed push reports the pull request as an explicit skip naming the unpublished branch, offers no pull-request retry, and opens no browser tab. |
| 198 | The action menu still offers Commit & push and Commit all changes, the confirmation promises a pull request only for the action that opens one, and the step list shows each action's own plan. |
| 199 | The module's Changes tab carries the same action footer, leading with Commit & push because a base checkout normally sits on the default branch, states that hooks always run and push never forces, lists that action's steps in run order before anything happens, keeps the pull-request stack and the commit-only action in its menu, and commits the whole change set through the module's own command without any task identifier. |
| 200 | Commit & push on the module checkout reads nothing from the remote until it is asked, then confirms that checkout's branch, remote and commit count with no generated commit text and no promise of a pull request, sends only the module once confirmed, reports all four steps' own outcomes in run order, and re-reads the module checkout afterwards. |
| 201 | A module base checkout parked on a feature branch can review a file's diff, confirm, and run the pull-request stack from the action menu: it reports all five steps' own outcomes, sends only the module, surfaces the pull request URL, and opens it in the system browser once. |
| 202 | A module base checkout on the repository's default branch is refused the pull request with its own precondition sentence, runs no step, opens no browser tab, and leaves the reviewed file list on screen. |
| 203 | A commit the module checkout's hooks refused shows the curated failure with the hook's captured output and commits nothing, and a diverged push reports a failed push step telling the user to resolve it in a terminal while stating that the commit is safe in this checkout rather than in a worktree. |
| 204 | A module action issues no worktree command, invalidating a task worktree in the same module leaves the module review and its spent confirmation alone, invalidating the module checkout repaints it, and re-opening the confirmation re-probes the remote instead of serving the previous count. |
| 205 | Ticket details let a top-level task create its opt-in worktree, show its branch, clean or dirty state, ahead and behind counts, explain integration conflicts, and require an explicit discard confirmation that warns about uncommitted changes. A subtask names the top-level task whose worktree it shares and offers no create action. |
| 206 | The footer Worktrees button opens a right-side panel without a clipping overflow ancestor and without changing the selected work item. The panel lists only the selected Module's persisted task worktrees with owner, branch, live clean or dirty, ahead, behind, and conflict state. It does not request live status for top-level tasks without a worktree, and reopening the panel while status is fresh does not refetch it. Discard uses the shared confirmation and stronger dirty warning, and create or discard on either the panel or ticket details refreshes both views without a reload. |
| 207 | Desktop Worktrees rows pass their worktree path through the Studio runtime to Open in Finder, while a browser runtime does not render that action. |
| 208 | A failed work-item read in the Worktrees panel stops loading, reports the failure in place, and does not request worktree status for the unreadable task. |
| 209 | Escape closes the Worktrees panel, consumes the key before Studio's global keymap can act on it, and restores focus to the Worktrees button. |
| 210 | The task worktree's changed files start as an expanded compact folder tree with normalized grouping, natural file order, root files, folder line totals, full-path accessibility, keyboard selection, selected-row state, and one in-place diff requested with the original path. |
| 211 | Changed-file folders are native buttons with their expanded state, one folder collapses without disturbing its siblings, the all-folders action preserves root files and switches modes, and refresh or invalidation drops expansion overrides for folder paths that disappeared. |
| 212 | Opening and reopening a task worktree's Changes tab checks its recorded pull request, Refresh checks again, an open pull request stays silent, and a merged pull request shows the exact linked cleanup prompt with a discard action. |
| 213 | A pull request closed without merging appears in a linked informational banner and offers no discard action. |
