...
pub fn parse(value: &str) -> Result<DateTime<Utc>, RunsPersistenceError> {
...
pub fn normalize(value: &str) -> Result<String, RunsPersistenceError> {
...
pub fn database_format(value: DateTime<Utc>) -> String {
    value.naive_utc().format("%Y-%m-%d %H:%M:%S%.f").to_string()
}

pub fn database_now() -> String {
