...
pub(crate) struct PendingBodies {
    bodies: Arc<Mutex<HashMap<String, Arc<Vec<u8>>>>>,
}

impl PendingBodies {
    pub(crate) fn hold(&self, operation_id: &str, bytes: Arc<Vec<u8>>) {
        self.entries().insert(operation_id.to_owned(), bytes);
    }

    pub(crate) fn get(&self, operation_id: &str) -> Option<Arc<Vec<u8>>> {
        self.entries().get(operation_id).cloned()
    }

    pub(crate) fn release(&self, operation_id: &str) {
