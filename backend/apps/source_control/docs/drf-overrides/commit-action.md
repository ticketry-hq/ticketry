...
- Service module / `transaction.atomic` used: `apps.source_control.commit` coordinates git through `apps.source_control.git_cli` and serializes writes per checkout through `apps.source_control.checkout_lock`. No transaction is used or wanted: the durable state changed here is the git repository, and holding a database transaction across a hook that may run for minutes would be worse than useless.
