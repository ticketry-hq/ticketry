...
pub struct LaunchConstructionRequest<'a> {
    ...
    pub hook_runner: &'a Path,
    pub hook_spool_directory: &'a Path,
    pub mcp_data_directory: &'a Path,
}
...
pub(crate) fn claude(
    ...
    let hook = hook_command(request, "claude", false);
    let settings = provider_settings(metadata, request.agent_run_id, &hook, None, true);
    let mcp = mcp_server(request, false, false);
    argv.splice(
        1..1,
        [
            "--settings".into(),
            ...
            "--mcp-config".into(),
            compact_json(&json!({"mcpServers": {"ticketry": mcp}})),
        ],
    );
...
pub(crate) fn codex(
    ...
    let hook = hook_command(request, "codex", true);
    let settings = provider_settings(metadata, request.agent_run_id, &hook, None, false);
    let mcp = mcp_server(request, true, false);
    let injected = [
        "-c".into(),
        format!("hooks={}", toml_inline(&settings["hooks"])),
        "-c".into(),
        format!("mcp_servers={}", toml_inline(&json!({"ticketry": mcp}))),
        "-c".into(),
...
pub(crate) fn gemini(
    ...
    let hook = hook_command(request, "gemini", true);
    let mcp = mcp_server(request, false, true);
    let settings = RuntimeSettings {
        environment_name: metadata
            .settings_environment
            .expect("Gemini settings environment"),
        contents: provider_settings(metadata, request.agent_run_id, &hook, Some(mcp), false),
    };
...
pub(crate) fn agy(
    ...
    let hook = hook_command(request, "agy", true);
    let mcp = mcp_server(request, false, true);
    let settings = RuntimeSettings {
        environment_name: metadata
            .settings_environment
...
fn validate(
    ...
    if request.agent_run_id.is_empty()
        || !request.working_directory.is_absolute()
        || !request.hook_runner.is_absolute()
        || !request.hook_spool_directory.is_absolute()
        || !request.mcp_data_directory.is_absolute()
    {
        return Err(ProviderError::new(
...
fn provider_settings(
    ...
    let hook = json!({"hooks": [{"type": "command", "command": hook_command, "timeout": metadata.hook_timeout}]});
    let hooks = metadata
        .hook_events
        .iter()
        ...
        .collect::<Map<_, _>>();
    if claude {
        json!({"env": {"MUXED_AGENT_RUN_ID": agent_run_id}, "hooks": hooks})
    } else if metadata.settings_environment.is_some() {
        json!({"hooks": hooks, "mcpServers": {"ticketry": mcp_server.expect("settings providers require MCP")}})
    } else {
        json!({"hooks": hooks})
    }
}
...
fn hook_command(
    ...
    let mut args = vec![
        request.hook_runner.to_string_lossy().into_owned(),
        "hook".into(),
        provider_slug.into(),
        "--spool-dir".into(),
        request.hook_spool_directory.to_string_lossy().into_owned(),
    ];
    if include_agent_run_id {
        args.extend(["--agent-run-id".into(), request.agent_run_id.into()]);
    }
    shell_join(&args)
}

fn mcp_server(request: &LaunchConstructionRequest<'_>, use_env_vars: bool, trusted: bool) -> Value {
    let mut server = json!({
        "command": request.hook_runner,
        "args": ["mcp", "--data-dir", request.mcp_data_directory, "--agent-run-id", request.agent_run_id],
    });
    if use_env_vars {
