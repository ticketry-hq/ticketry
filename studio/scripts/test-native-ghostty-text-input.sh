...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ticketry-native-text-input.XXXXXX")
...
clang \
  ...
  -I "$INCLUDE_DIR" \
  -framework AppKit \
  "$STUDIO_DIR/src-tauri/native/tests/libghostty_text_input_tests.m" \
  -o "$TEST_DIR/libghostty_text_input_tests"

"$TEST_DIR/libghostty_text_input_tests"
