...
def _mk(task_id="t1", **over):
    ...
    fields.update(over)
    return Worktree.objects.create_for_task(**fields)


def test_create_and_get_round_trip():
    created = _mk(project_id="p1", module_id="m1", ticket_seq=5)
    fetched = Worktree.objects.get_by_task("t1")
...
def test_set_status_bumps_updated_at():
    created = _mk("t1")
    assert Worktree.objects.set_status("t1", "conflict") is True
    row = Worktree.objects.get_by_task("t1")
...
def test_delete():
    _mk("t1")
    Worktree.objects.delete_for_task("t1")
...
def test_list_by_scope():
    ...
    assert {w.task_id for w in Worktree.objects.by_scope(project_id="p1")} == {"t1", "t2"}
    assert {w.task_id for w in Worktree.objects.by_scope(module_id="m2")} == {"t2"}
    assert len(Worktree.objects.by_scope()) == 3


def test_get_missing_returns_none():
    assert Worktree.objects.get_by_task("nope") is None
...
def test_get_by_task_does_not_close_connection_mid_transaction():
    ...
    with transaction.atomic():
        Worktree.objects.get_by_task("t1")
    ...
    assert Worktree.objects.get_by_task("t2") is not None
