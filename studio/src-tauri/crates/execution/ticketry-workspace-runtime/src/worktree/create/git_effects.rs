...
use crate::worktree::status::registry::{self, same_path};
...
pub async fn head(git: &GitPort, repository: &Path) -> Result<RepositoryHead, WorktreeCreateError> {
...
mod tests {
    ...
    async fn a_repository_without_the_checkout_or_branch_is_clear() {
        ...
        crate::worktree::create::test_support::repository(&repository);
    ...
    async fn an_existing_branch_is_never_reused_for_a_new_checkout() {
        ...
        let repository = directory.path().join("repository");
        crate::worktree::create::test_support::repository(&repository);
        crate::worktree::create::test_support::git(&["branch", "wt/CODIN-1-task"], &repository);
    ...
    async fn creation_is_recognised_as_the_exact_intended_checkout() {
        let directory = tempfile::tempdir().expect("create a repository directory");
        let repository = directory.path().join("repository");
        let base = crate::worktree::create::test_support::repository(&repository);
        let checkout = directory.path().join("checkouts/CODIN-1-task");
        let git = GitPort::new();
    ...
    async fn a_detached_head_records_its_commit_as_the_base() {
        ...
        let repository = directory.path().join("repository");
        let base = crate::worktree::create::test_support::repository(&repository);
        crate::worktree::create::test_support::git(&["checkout", "--detach"], &repository);
