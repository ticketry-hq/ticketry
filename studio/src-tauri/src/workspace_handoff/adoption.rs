...
pub async fn adopt(data_directory: &Path) -> Result<HandoffEvidence, WorkspaceHandoffError> {
    ...
    crate::workspace_operations::schema::install(&database)
        .await
        .map_err(|error| {
            unknown(format!(
    ...
    crate::documents_persistence::preflight(data_directory)
        .await
        .map_err(|error| {
            unknown(format!(
    ...
    let documents = crate::documents_persistence::adopt(data_directory)
        .await
        .map_err(|error| {
            unknown(format!(
    ...
    crate::worktree_persistence::preflight(data_directory)
        .await
        .map_err(|error| {
            unknown(format!(
    ...
    let worktrees = crate::worktree_persistence::adopt(data_directory)
        .await
        .map_err(|error| {
            unknown(format!(
...
async fn open(
    data_directory: &Path,
) -> Result<sea_orm::DatabaseConnection, WorkspaceHandoffError> {
