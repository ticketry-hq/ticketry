...
/// Attach policy: duplicate viewers for a still-attached run are rejected.
/// A closed viewer remains queryable by its opaque handle, while a later
/// attach for that run receives a fresh handle.
#[tauri::command]
pub fn viewer_attach(
    ...
    let runtime = state.0.clone();
    {
        ...
        if registry.active_runs.contains_key(&run_id) {
            return Err(ViewerCommandError::DuplicateAttach { run_id });
        }
    }
    ...
    let (command_sender, command_receiver) = mpsc::channel();
    {
        let mut registry = runtime
            ...
            .lock()
            .expect("viewer registry lock poisoned");
        // The lock is released during core attachment. A simultaneous caller
        // can only win one insertion; the other viewer is detached immediately.
        if registry.active_runs.contains_key(&run_id) {
            drop(registry);
            let _ = viewer.detach();
            return Err(ViewerCommandError::DuplicateAttach { run_id });
        }
        registry.active_runs.insert(run_id, handle.clone());
...
fn close_viewer(
    ...
    let run_id = entry.status.run_id.clone();
    let status = entry.status.clone();
    registry.active_runs.remove(&run_id);
    status
}
