...
try {
  ...
  await assertSameTree(
    join(first, "entities"),
    join(studioRoot, "src-tauri/crates/foundation/ticketry-entities/src/foundation"),
    "SeaORM entity drift",
  );
  ...
  for (const name of ["graph_run.rs", "launch_claim.rs"]) {
    await assertSameFile(
      ...
      join(studioRoot, "src-tauri/crates/foundation/ticketry-entities/src/execution", name),
      `Execution ${name} entity drift`,
    );
  }
  await assertSameFile(
    join(first, "terminal-entities/session.rs"),
    join(studioRoot, "src-tauri/crates/foundation/ticketry-entities/src/terminals/session.rs"),
