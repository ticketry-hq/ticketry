...
| `transition_state` (`backend/worktracker/workflow.py:73`) | Sets the root state, toggles root archive status on entry to or exit from cancelled, and optionally records a forced transition (`backend/worktracker/workflow.py:140`, `backend/worktracker/workflow.py:147`, `backend/worktracker/workflow.py:151`). | Only when the assigned target changes state identity (`backend/worktracker/models/issue.py:137`). | Only when the assigned target changes state identity (`backend/worktracker/signals.py:122`, `backend/worktracker/signals.py:127`, `backend/apps/runs/signals.py:42`). | The root’s latest surviving state revision appears when it falls inside the replay interval (`backend/apps/runs/projections.py:78`). |
