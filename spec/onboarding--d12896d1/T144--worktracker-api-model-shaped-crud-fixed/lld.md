...
### 1.4 delete_state simplification (`services/workflow_config.py`)
...
ROOT_URLCONF) — add `rest_framework`, `drf_spectacular`.
...
| IssueTypeTransition | list under issue type (new canonical read), create, patch (agent_allowed), delete (prunes) |
| LaunchBinding | list under project, PUT/DELETE by (issue_type, state) composite key; auto_start + subtree_run_enabled are ordinary fields on the write |
