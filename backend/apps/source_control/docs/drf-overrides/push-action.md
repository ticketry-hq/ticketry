...
- Why an existing service function is insufficient: `apps.source_control.actions.stacked_action` remains the service and owns the full operation. The ViewSet only validates and serializes it.
- Smallest custom seam: `WorktreePushViewSet.commit_push`, `ModulePushViewSet.commit_push`, and their read-only preview actions.
- Service module / `transaction.atomic` used: `apps.source_control.actions.stacked_action` serializes the Git steps. `apps.source_control.records.ship_records` uses a short transaction after Git settles.
