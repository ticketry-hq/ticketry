...
use crate::planning::{ModulePromptFacts, TaskPromptFacts, TaskSummary};
...
pub(super) async fn project_slug(
    database: &DatabaseConnection,
    project_id: &str,
) -> Result<String, LaunchAuthorityError> {
    project::Entity::find_by_id(compact(project_id))
        .one(database)
        .await?
        .map(|row| row.slug)
...
pub(super) async fn module_prompt_facts(
    ...
    Ok(ModulePromptFacts {
        ...
        project_slug: project_slug(database, &module.project_id).await?,
