...
use crate::installation_classification::{self, Installation};
use crate::installation_preflight::{self, Verdict};
...
pub async fn adopt_with(
    ...
    let classified = installation_classification::classify(data_directory)
        .await
        .map_err(|error| {
            AdoptionFailure::new(
    ...
    match &classified {
        ...
        Installation::PostgresImportSource(source) => {
            let staged = crate::installation_import::stage(data_directory, source, plan).await?;
            ...
            match adopted {
                Ok(adopted) => {
                    crate::installation_import::activate(data_directory, staged).map(|()| adopted)
                }
...
async fn first_launch(
    ...
    let provisioned = installation_classification::classify(data_directory)
        .await
        .map_err(|error| {
            AdoptionFailure::new(
                Phase::Provisioning,
...
async fn existing(
    ...
    let report = installation_preflight::preflight(data_directory, classified)
        .await
        .map_err(|error| {
            AdoptionFailure::new(
                Phase::Preflight,
