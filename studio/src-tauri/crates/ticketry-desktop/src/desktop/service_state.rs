...
pub const HEALTH_EVENT: &str = "desktop-service-health";
...
pub struct DesktopServiceState {
    ...
    >,
    pub hook_spool_runtime: Mutex<Option<ticketry_runs::hook_spool::HookSpoolRuntime>>,
    pub execution_runtime: Mutex<
        Option<ticketry_agent_execution::execution::reconciliation::ExecutionReconciliationRuntime>,
    >,
    pub terminal_launch: Mutex<Option<ticketry_terminal::terminal::launch::TerminalLaunchService>>,
    pub output_sweep:
    ...
    pub notice_ids: Mutex<HashSet<String>>,
    pub readiness: ReadinessPublication,
...
impl DesktopServiceState {
    ...
    pub fn record_health(&self, health: ServiceHealth) {
        *self.health.lock().expect("service health lock poisoned") = health;
    }

    pub fn publish(&self, application: &tauri::AppHandle, health: ServiceHealth) {
        self.record_health(health.clone());
        let _ = application.emit(HEALTH_EVENT, health);
    }

    pub fn retain_notice(&self, notice: UserNotice) -> bool {
        let inserted = self
            .notice_ids
            .lock()
