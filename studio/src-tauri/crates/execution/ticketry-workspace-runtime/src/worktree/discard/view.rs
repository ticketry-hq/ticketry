...
use crate::worktree::status::WorktreeStatusView;
...
impl WorktreeDiscardResult {
    pub fn removed(branch: Option<String>, status: WorktreeStatusView) -> Self {
        Self {
            removed: true,
    ...
    pub fn settled(result: &serde_json::Value, status: WorktreeStatusView) -> Self {
    ...
    pub fn absent(status: WorktreeStatusView) -> Self {
        Self {
            removed: false,
            task_id: status.task_id.clone(),
            top_level_task_id: status.top_level_task_id.clone(),
