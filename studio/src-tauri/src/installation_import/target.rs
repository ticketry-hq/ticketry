...
async fn copy(
    ...
    let source_product = source_table_set
        .iter()
        .filter(|table| crate::installation_classification::schema_facts::is_product_table(table))
        .cloned()
        .collect::<BTreeSet<_>>();
...
async fn bridge_to_current(
    ...
    if source_generation == crate::installation_classification::manifest().current_generation {
        return Ok(Vec::new());
    }
    let bridge =
        crate::installation_adoption::bridge::select(source_generation, source_fingerprint)?;
    database
        .execute_unprepared("PRAGMA foreign_keys = OFF")
        .await
        .map_err(|error| {
    ...
    let transaction = database
        ...
        .await
        .map_err(|error| target_error(error.to_string()))?;
    let outcome = crate::installation_adoption::bridge::apply(&transaction, &[bridge]).await;
    match outcome {
        Ok(()) => transaction
            .commit()
            .await
...
async fn promote_migration_ledger(
    ...
    let manifest = crate::installation_classification::manifest();
...
async fn validate(directory: &Path, imported: &Inventory) -> Result<(), AdoptionFailure> {
    let classified = crate::installation_classification::classify(directory)
        .await
        .map_err(|error| target_error(format!("the staged target does not classify: {error}")))?;
    let report = crate::installation_preflight::preflight(directory, &classified)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    crate::installation_adoption::integrity::structural(&database)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    let present = crate::installation_adoption::inventory::product_tables(&database)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    let unanswered = crate::installation_adoption::representative_reads::prove(&database, &present)
        .await
...
async fn open_existing(path: &Path) -> Result<DatabaseConnection, AdoptionFailure> {
    crate::installation_adoption::exclusive::open_shared(path).await
}
