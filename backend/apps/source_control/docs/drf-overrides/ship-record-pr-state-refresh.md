...
- Service module / `transaction.atomic` used: `apps.source_control.pull_request_state` owns the one bounded `gh` call. `apps.source_control.ship_record_refresh` opens a short transaction only after that call succeeds and updates `pr_state` plus `pr_refreshed_at`.
