...
/// identity exists yet.
pub const PRE_COMMIT_STAGES: [&str; 9] = [
    "launch-requested",
    "launch-policy-evaluated",
    "launch-authority-resolved",
    "launch-provider-validated",
    "launch-executable-resolved",
    "launch-directory-preflighted",
    "launch-argv-materialised",
    "terminal-runtime-spawned",
    "prompt-delivered",
];

/// Stages from the commit onwards.
///
/// `launch-attempt-committed` is the join: it is the one record carrying both
/// the launch attempt identity and the Agent Run identity that attempt
/// produced, which is what lets the reader read both halves as one path. The
/// stages after it are the launch-discovery stages, unchanged.
pub const POST_COMMIT_STAGES: [&str; 10] = [
...
mod tests {
    ...
    fn path_order_runs_from_the_request_through_to_the_workspace_render() {
        ...
        assert_eq!(
            ...
            Some(PRE_COMMIT_STAGES.len() + POST_COMMIT_STAGES.len() - 1),
    ...
    fn only_the_stages_before_the_commit_precede_an_agent_run_identity() {
        assert!(is_pre_commit_stage("prompt-delivered"));
