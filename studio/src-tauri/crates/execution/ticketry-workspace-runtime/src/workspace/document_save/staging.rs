...
pub fn staging_path(directory: &Path, operation_id: &str) -> PathBuf {
...
pub fn staged_operation(name: &str) -> Option<String> {
    let identity = name
...
pub fn staging_files(directory: &Path) -> Vec<(PathBuf, String)> {
...
pub fn staged_digest(path: &Path) -> Option<String> {
    std::fs::read(path)
        .ok()
...
pub fn stage(path: &Path, bytes: &[u8]) -> io::Result<()> {
    use std::io::Write;
...
pub fn commit(staged: &Path, target: &Path) -> io::Result<()> {
