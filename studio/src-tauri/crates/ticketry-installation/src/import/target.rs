...
async fn copy(
    ...
    let source_product = source_table_set
        .iter()
        .filter(|table| crate::classification::schema_facts::is_product_table(table))
        .cloned()
        .collect::<BTreeSet<_>>();
...
async fn bridge_to_current(
    ...
    let transaction = database
        ...
        .await
        .map_err(|error| target_error(error.to_string()))?;
    let outcome = crate::adoption::bridge::apply(&transaction, &[bridge]).await;
    match outcome {
        Ok(()) => transaction
            .commit()
            .await
...
async fn promote_migration_ledger(
    ...
    let now = crate::adoption::now_rfc3339();
    let transaction = database
        .begin()
        .await
        .map_err(|error| target_error(error.to_string()))?;
...
async fn validate(directory: &Path, imported: &Inventory) -> Result<(), AdoptionFailure> {
    let classified = crate::classification::classify(directory)
        .await
        .map_err(|error| target_error(format!("the staged target does not classify: {error}")))?;
    let report = crate::preflight::preflight(directory, &classified)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    crate::adoption::integrity::structural(&database)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    let present = crate::adoption::inventory::product_tables(&database)
        .await
        .map_err(|error| {
            target_error(format!(
    ...
    let unanswered = crate::adoption::representative_reads::prove(&database, &present)
        .await
...
async fn open_existing(path: &Path) -> Result<DatabaseConnection, AdoptionFailure> {
    crate::adoption::exclusive::open_shared(path).await
}
