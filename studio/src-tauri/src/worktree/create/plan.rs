...
pub(crate) async fn for_owner(
    ...
    Ok(CreatePlan {
        ...
        workspace_slug: project_slug(work_items, &top_level.project_id).await?,
        owner,
    })
}

/// The owning project's slug, recorded on the worktree row as metadata under
/// its legacy `workspace_slug` column name. Workspace is gone, so the
/// installation project is the only slug a checkout can belong to. It is
/// derived from the Work Item graph, never from a caller and never from local
/// configuration.
async fn project_slug(
    work_items: &DatabaseConnection,
    project_id: &str,
) -> Result<Option<String>, WorktreeCreateError> {
    Ok(project::Entity::find_by_id(project_id.to_owned())
        .one(work_items)
        .await?
        .map(|row| row.slug)
