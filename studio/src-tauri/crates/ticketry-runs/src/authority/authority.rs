...
use crate::authority::grant_store::{GrantStore, StoredGrant};
...
impl RunAuthority {
    ...
    /// Issue a grant directly, skipping the launch path that normally mints one.
    ///
    /// Only the MCP authorization tests need this, and they live in another
    /// crate, so `#[cfg(test)]` cannot reach it.
