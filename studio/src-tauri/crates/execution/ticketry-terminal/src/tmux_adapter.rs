...
pub use tool_path::approved_tool_path;
pub(crate) use tool_path::resolve_approved_tool_path;
...
impl TmuxAdapter {
    ...
    pub fn validate_run_id(run_id: &str) -> Result<(), TmuxAdapterError> {
        validate_identifier(run_id)
    }

    pub fn session_exists(&self, run_id: &str) -> Result<bool, TmuxAdapterError> {
        validate_identifier(run_id)?;
    ...
    pub fn capture_screen(&self, run_id: &str) -> Result<Vec<u8>, TmuxAdapterError> {
        validate_identifier(run_id)?;
        let session = session_name(run_id);
        let output = self
    ...
    pub fn resize(&self, run_id: &str, columns: u16, rows: u16) -> Result<(), TmuxAdapterError> {
    ...
    pub fn validate_input(input: &[u8]) -> Result<(), TmuxAdapterError> {
    ...
    pub fn attach_command(&self, run_id: &str) -> CommandBuilder {
    ...
    pub fn attach_shell_command(&self, run_id: &str, renderer_environment: &[String]) -> String {
