...
describe("overhaul acceptance - modal chunk load failure", () => {
  ...
  it("[overhaul-213] surfaces a recoverable panel instead of blanking the app when the settings chunk fails", async () => {
  ...
  it("[overhaul-214] retries the failed chunk and shows the real settings dialog", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    chunk.failNextLoad = true;
  ...
  it("[overhaul-215] closes the failed modal without blanking the app", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    chunk.failNextLoad = true;
  ...
  it("[overhaul-216] shows a visible loading status while the modal chunk is pending", async () => {
    render(<ModalHost />);
  ...
  it("[overhaul-217] renders nothing when the modal stack is empty", () => {
