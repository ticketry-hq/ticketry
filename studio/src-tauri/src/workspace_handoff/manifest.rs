...
pub const OWNED_TABLES: &[(&str, ProductionWriter, &[&str])] = &[
    (
        ...
        crate::documents_persistence::ownership_manifest::DESIGN_DOCUMENT_COLUMNS,
    ),
    (
        crate::documents_persistence::LEDGER_TABLE,
        ProductionWriter::Documents,
        ADOPTION_LEDGER_COLUMNS,
    ),
    (
        crate::worktree_persistence::ADOPTED_TABLE,
        ProductionWriter::Worktrees,
        WORKTREE_COLUMNS,
    ),
    (
        crate::worktree_persistence::LEDGER_TABLE,
        ProductionWriter::Worktrees,
        ADOPTION_LEDGER_COLUMNS,
    ),
    (
...
mod tests {
    ...
    fn the_composition_covers_every_capability_manifest() {
        for table in crate::documents_persistence::ownership_manifest::owned_tables() {
            assert_eq!(production_writer(table), Some(ProductionWriter::Documents));
        }
        for table in crate::worktree_persistence::ownership_manifest::owned_tables() {
            assert_eq!(production_writer(table), Some(ProductionWriter::Worktrees));
        }
        for table in crate::workspace_operations::ownership_manifest::owned_tables() {
            assert_eq!(
    ...
    fn both_adoption_ledgers_are_owned_alongside_their_table() {
        assert_eq!(
            production_writer(crate::documents_persistence::LEDGER_TABLE),
            Some(ProductionWriter::Documents)
        );
        assert_eq!(
            production_writer(crate::worktree_persistence::LEDGER_TABLE),
